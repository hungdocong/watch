...
namespace Watch.Models.Business
{
    public class ProductBusiness
    {
        ...
        public List<Product> getRandomProduct()
        {
            ...
            if (index == 1)
                lst_pro = db.Products.Where(x => x.Status == true).OrderByDescending(x => x.Product_Name).ToList();
            else if(index == 2)
                lst_pro = db.Products.Where(x => x.Status == true).OrderBy(x => x.Product_Name).ToList();
            else if (index == 3)
                lst_pro = db.Products.Where(x => x.Status == true).OrderByDescending(x => x.ID).ToList();
            else
                lst_pro = db.Products.Where(x => x.Status == true).OrderBy(x => x.ID).ToList();
        ...
        public IEnumerable<Product> getProductByCategoryID(long? category_id)
        {
                ...
                return db.Products.Where(x => x.Category_ID == category_id && x.Status == true).OrderByDescending(x => x.Product_Name).ToList();
